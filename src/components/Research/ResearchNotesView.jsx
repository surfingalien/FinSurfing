...
import {
  ...
  Map, Sun, Layers, AlertTriangle, Zap, ExternalLink,
} from 'lucide-react'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="glass rounded-2xl w-full max-w-2xl border border-indigo-500/25 shadow-2xl">
        ...
        <div className="p-4 space-y-3">
          {!result ? (
            <>
              <p className="text-xs text-slate-500">Dump your raw thoughts — AI will structure them into a clean research note.</p>
              <textarea
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={structureThoughts} disabled={!raw.trim() || processing}
                  ...
                  {processing ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> Structuring…</> : <><Sparkles className="w-3.5 h-3.5" /> Structure with AI</>}
                </button>
              </div>
            </>
          ) : (
            <>
              <div className="flex items-center gap-2"><Sparkles className="w-3.5 h-3.5 text-mint-400" /><span className="text-xs font-semibold text-mint-400">Structured result</span><TypeBadge type={result.note_type} /></div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { onSave({ title: result.title, content: result.content, tags: result.tags || [], note_type: result.note_type || 'braindump', symbol }); onClose() }}
