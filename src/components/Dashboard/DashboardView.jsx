...
function FearGreedGauge({ fg }) {
  ...
  const [ax, ay] = pt(needleDeg)
  const [nx, ny] = pt(needleDeg)  // needle tip (slightly shorter)
  const needleLen = r - 12
  const nxs = +(cx + needleLen * Math.cos((needleDeg * Math.PI) / 180)).toFixed(2)
  const nys = +(cy - needleLen * Math.sin((needleDeg * Math.PI) / 180)).toFixed(2)

  const zoneColors = ['#7c3aed', '#6366f1', '#f59e0b', '#f97316', '#ef4444']
  const zoneDeg    = [180, 144, 108, 72, 36, 0]

  return (
    <div>
      <svg width="200" height="100" viewBox="0 0 200 100" className="mx-auto block">
        {/* Background track */}
        <path d={`M ${lx} ${ly} A ${r} ${r} 0 0 1 ${rx} ${ry}`}
          fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="14" strokeLinecap="round" />

        {/* Colored zone arcs */}
        {zoneDeg.slice(0, -1).map((from, i) => {
          const to = zoneDeg[i + 1]
          const [x1, y1] = pt(from), [x2, y2] = pt(to)
          return (
            <path key={i}
              d={`M ${x1} ${y1} A ${r} ${r} 0 0 1 ${x2} ${y2}`}
              fill="none" stroke={zoneColors[i]} strokeWidth="12" strokeOpacity="0.5" />
          )
        })}

        {/* Active progress arc */}
        {pct > 0 && (
          <path d={`M ${lx} ${ly} A ${r} ${r} 0 ${pct > 0.5 ? 1 : 0} 1 ${ax} ${ay}`}
            fill="none" stroke={color} strokeWidth="10" strokeLinecap="round"
            style={{ filter: `drop-shadow(0 0 6px ${color}80)` }} />
        )}

        {/* Needle */}
        <line x1={cx} y1={cy} x2={nxs} y2={nys}
        ...
        <text x={cx} y={cy - 8} textAnchor="middle" fill="white"
          fontSize="22" fontWeight="900" fontFamily="ui-monospace, monospace">{score}</text>
        <text x={cx} y={cy + 8} textAnchor="middle" fill={color}
          fontSize="9" fontWeight="700" letterSpacing="0.08em">{label.toUpperCase()}</text>
      </svg>
        ...
        <div className="mt-3 space-y-1.5 border-t border-white/[0.06] pt-3">
          {components.map((c, i) => (
            <div key={i} className="flex items-center justify-between text-xs">
              ...
              <span className={`font-bold font-mono w-6 text-right text-[10px] ${c.score > 50 ? 'text-emerald-400' : 'text-red-400'}`}>
                {c.score}
              </span>
            </div>
          ))}
