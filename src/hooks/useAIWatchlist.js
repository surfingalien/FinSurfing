...
export function useAIWatchlist() {
  ...
  const addStock = useCallback((item) => {
    ...
    const normalized = {
      ...
      entryZoneLow:    item.entryZoneLow    ?? null,
      entryZoneHigh:   item.entryZoneHigh   ?? null,
      takeProfitPrice: item.takeProfitPrice ?? null,
      targetZoneLow:   item.targetZoneLow   ?? null,
      targetZoneHigh:  item.targetZoneHigh  ?? null,
      stopLossPrice:   item.stopLossPrice   ?? null,
      stopZoneLow:     item.stopZoneLow     ?? null,
      stopZoneHigh:    item.stopZoneHigh    ?? null,
