...
1. **Dry-run output**: Display the complete skill content that will be written
2. **Proposed location**: Where the skill will be saved
3. **Similar skills found**: Any near-matches from deduplication check
4. **Ask for confirmation**: "Create this skill? [y/n/edit]"
...
2. **Check for similar names**: Look for skills with overlapping keywords
3. **Show near-matches**: If similar skills exist, ask user: "Update existing skill X, or create new?"
4. **Prefer updates**: When knowledge extends an existing skill, update it with a version bump
