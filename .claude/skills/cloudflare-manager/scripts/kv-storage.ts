...
async function writeKey(
  ...
  try {
    ...
    await makeApiRequestWithRetry(
      `/accounts/${accountId}/storage/kv/namespaces/${encodePathSegment(namespaceId)}/values/${encodePathSegment(key)}`,
      {
        method: 'PUT',
        body: value,
...
async function listKeys(namespaceId: string, limit: number = 100): Promise<void> {
  try {
    ...
    const response = await makeApiRequestWithRetry<{ keys: KVKey[]; list_complete: boolean }>(
      `/accounts/${accountId}/storage/kv/namespaces/${encodePathSegment(namespaceId)}/keys`,
      {
...
async function bulkDelete(namespaceId: string, keys: string[]): Promise<void> {
  try {
    ...
    await makeApiRequestWithRetry(
      `/accounts/${accountId}/storage/kv/namespaces/${encodePathSegment(namespaceId)}/bulk`,
      {
        method: 'DELETE',
        body: keys,
      }
...
Usage:
  bun scripts/kv-storage.ts create-namespace <name>
  bun scripts/kv-storage.ts list-namespaces
  bun scripts/kv-storage.ts delete-namespace <namespace-id> [--force]
  bun scripts/kv-storage.ts write <namespace-id> <key> <value>
  bun scripts/kv-storage.ts read <namespace-id> <key>
  bun scripts/kv-storage.ts delete <namespace-id> <key> [--force]
  bun scripts/kv-storage.ts list-keys <namespace-id>
  bun scripts/kv-storage.ts bulk-write <namespace-id> <json-file>
  bun scripts/kv-storage.ts bulk-delete <namespace-id> <key1> [key2...] [--force]
...
Examples:
  ...
  switch (command) {
    ...
    case 'delete-namespace': {
      ...
      if (!await confirmDestructive(`delete KV namespace ${namespaceId}`, !!flags.force)) {
        process.exit(1);
      }
    ...
    case 'delete': {
      ...
      if (!await confirmDestructive(`delete key "${key}" from namespace ${namespaceId}`, !!flags.force)) {
        process.exit(1);
      }
    ...
    case 'bulk-delete': {
      ...
      if (!await confirmDestructive(`bulk delete ${keys.length} keys from namespace ${namespaceId}`, !!flags.force)) {
        process.exit(1);
      }
