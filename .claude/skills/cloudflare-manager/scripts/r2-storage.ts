...
async function deleteBucket(name: string): Promise<void> {
  try {
    ...
    await makeApiRequestWithRetry(
      `/accounts/${accountId}/r2/buckets/${encodePathSegment(name)}`,
      { method: 'DELETE' }
    );
...
async function uploadObjectWithCurl(
  ...
  const url = `https://api.cloudflare.com/client/v4/accounts/${accountId}/r2/buckets/${encodePathSegment(bucketName)}/objects/${encodePathSegment(objectKey)}`;
...
async function deleteObject(bucketName: string, objectKey: string): Promise<void> {
  try {
    ...
    await makeApiRequestWithRetry(
      `/accounts/${accountId}/r2/buckets/${encodePathSegment(bucketName)}/objects/${encodePathSegment(objectKey)}`,
      { method: 'DELETE' }
    );
...
async function listObjects(bucketName: string, prefix?: string): Promise<void> {
  try {
    ...
    const response = await makeApiRequestWithRetry<{ objects: R2Object[] }>(
      `/accounts/${accountId}/r2/buckets/${encodePathSegment(bucketName)}/objects`,
      { queryParams }
    );
...
Usage:
  bun scripts/r2-storage.ts create-bucket <name>
  bun scripts/r2-storage.ts list-buckets
  bun scripts/r2-storage.ts delete-bucket <name> [--force]
  bun scripts/r2-storage.ts upload <bucket> <local-path> <object-key>
  bun scripts/r2-storage.ts download <bucket> <object-key> <local-path>
  bun scripts/r2-storage.ts delete-object <bucket> <object-key> [--force]
  bun scripts/r2-storage.ts list-objects <bucket> [--prefix <prefix>]
...
Examples:
  ...
  switch (command) {
    ...
    case 'delete-bucket': {
      ...
      if (!await confirmDestructive(`delete R2 bucket ${name}`, !!flags.force)) {
        process.exit(1);
      }
    ...
    case 'delete-object': {
      ...
      if (!await confirmDestructive(`delete object "${objectKey}" from bucket ${bucket}`, !!flags.force)) {
        process.exit(1);
      }
