  ...
  try {
    ...
    try {
      const response = await makeApiRequestWithRetry<PagesProject>(
        `/accounts/${accountId}/pages/projects/${encodePathSegment(projectName)}`
      );
...
async function listDeployments(projectName: string): Promise<void> {
  try {
    ...
    const response = await makeApiRequestWithRetry<PagesDeployment[]>(
      `/accounts/${accountId}/pages/projects/${encodePathSegment(projectName)}/deployments`
    );
...
Usage:
  ...
  bun scripts/pages.ts list
  bun scripts/pages.ts get <project-name>
  bun scripts/pages.ts delete <project-name> [--force]
...
Note: For actual file uploads, use Wrangler CLI:
  ...
  switch (command) {
    ...
    case 'delete': {
      ...
      if (!await confirmDestructive(`delete Pages project ${projectName}`, !!flags.force)) {
        process.exit(1);
      }
