...
async function deleteDnsRecord(zoneName: string, recordId: string): Promise<void> {
  try {
    ...
    await makeApiRequestWithRetry(
      `/zones/${zoneId}/dns_records/${encodePathSegment(recordId)}`,
      { method: 'DELETE' }
    );
...
async function deleteWorkerRoute(zoneName: string, routeId: string): Promise<void> {
  try {
    ...
    await makeApiRequestWithRetry(
      `/zones/${zoneId}/workers/routes/${encodePathSegment(routeId)}`,
      { method: 'DELETE' }
    );
...
Usage:
  ...
  bun scripts/dns-routes.ts list-dns <zone-name>
  bun scripts/dns-routes.ts delete-dns <zone-name> <record-id> [--force]
  bun scripts/dns-routes.ts create-route <zone-name> <pattern> <worker-name>
  bun scripts/dns-routes.ts list-routes <zone-name>
  bun scripts/dns-routes.ts delete-route <zone-name> <route-id> [--force]
...
Examples:
  ...
  switch (command) {
    ...
    case 'delete-dns': {
      ...
      if (!await confirmDestructive(`delete DNS record ${recordId} in zone ${zoneName}`, !!flags.force)) {
        process.exit(1);
      }
    ...
    case 'delete-route': {
      ...
      if (!await confirmDestructive(`delete worker route ${routeId} in zone ${zoneName}`, !!flags.force)) {
        process.exit(1);
      }
