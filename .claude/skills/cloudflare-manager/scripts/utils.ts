...
export async function makeApiRequest<T = any>(
  ...
  if (body !== undefined && method !== 'GET' && !isRawBody && !requestHeaders['Content-Type']) {
  ...
  if (body !== undefined && method !== 'GET') {
    requestOptions.body = isRawBody ? body : JSON.stringify(body);
  }
 ...
 */
export async function confirmDestructive(action: string, force = false): Promise<boolean> {
  if (force) {
    return true;
  }

  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    printWarning(`Refusing to ${action} in a non-interactive shell. Re-run with --force to continue.`);
    return false;
  }
