...
async function deployWorker(
  ...
  try {
    ...
    // The Workers API needs multipart/form-data, so we use curl for upload.
...
async function deployWorkerWithCurl(
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'cf-worker-'));
  const tmpScript = join(tempDir, `${name}.js`);
  const tmpMetadata = join(tempDir, `${name}-metadata.json`);

  try {
    writeFileSync(tmpScript, scriptContent, { encoding: 'utf-8', mode: 0o600 });
    writeFileSync(tmpMetadata, JSON.stringify(metadata), { encoding: 'utf-8', mode: 0o600 });

    const url = `https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/scripts/${encodePathSegment(name)}`;
...
/**
 * Get the account subdomain used for workers.dev URLs.
 */
async function getWorkersSubdomain(accountId: string): Promise<string | undefined> {
  try {
 ...
 */
async function getWorkerUrl(accountId: string, name: string, subdomain?: string): Promise<string> {
  const resolvedSubdomain = subdomain ?? await getWorkersSubdomain(accountId);

  if (resolvedSubdomain) {
    return `https://${name}.${resolvedSubdomain}.workers.dev`;
  }
...
async function getWorker(name: string): Promise<void> {
  try {
    ...
    const response = await makeApiRequestWithRetry<WorkerScript>(
      `/accounts/${accountId}/workers/scripts/${encodePathSegment(name)}`
    );
...
async function listWorkers(): Promise<void> {
  try {
    ...
    const subdomain = await getWorkersSubdomain(accountId);

    for (const worker of workers) {
      const url = await getWorkerUrl(accountId, worker.id, subdomain);
...
Usage:
  ...
  bun scripts/workers.ts get <name>
  bun scripts/workers.ts list
  bun scripts/workers.ts delete <name> [--force]
  bun scripts/workers.ts logs <name> [--tail]
  bun scripts/workers.ts get-url <name>
...
Examples:
  ...
  switch (command) {
    ...
    case 'delete': {
      ...
      if (!await confirmDestructive(`delete worker ${name}`, !!flags.force)) {
        process.exit(1);
      }
