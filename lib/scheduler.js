...
async function trigger(id) {
  ...
  try {
    ...
    RESULTS.set(id, {
      status: 'done', lastRun: Date.now(), data, error: null,
      durationMs: Date.now() - startedAt, failedAt: null,
    })
    emitter.emit('status', { id, status: 'done', data })
    return data
  } catch (e) {
    // Log the STACK, not just the message: these jobs run unattended, and a
    // bare message ("fetch failed", "HTTP 500") gives no way to tell which
    // internal call broke. This is the only record of why a job failed.
    console.error(`[scheduler] job "${id}" FAILED after ${Date.now() - startedAt}ms:`, e.stack || e.message)
    RESULTS.set(id, {
      status: 'failed', lastRun: Date.now(), data: null, error: e.message,
      durationMs: Date.now() - startedAt, failedAt: new Date().toISOString(),
    })
...
function getStatus() {
  return [...JOBS.values()].map(job => ({
    ...
    enabled:     job.enabled,
    result:      RESULTS.get(job.id) ?? { status: 'idle', lastRun: null, data: null, error: null, durationMs: null, failedAt: null },
  }))
}
