...
async function morningBriefEmail() {
  ...
  const [macroData, brainData] = await Promise.allSettled([
    internalGet('/api/macro/indicators', 30_000),
    internalPost('/api/ai-brain/analyze', { scanMode: 'broad', horizon: '3m' }, 120_000),
  ]).then(([m, b]) => [
    m.status === 'fulfilled' ? m.value : null,
    b.status === 'fulfilled' ? b.value : null,
  ])

  if (!brainData) throw new Error('AI Brain analyze failed — no data returned')
