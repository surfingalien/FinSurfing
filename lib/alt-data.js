 ...
 * getAltDataSnippet(symbol)  — Combined prompt-injection string (30-min cache)
 */

const TIMEOUT = 8_000

// 30-minute TTL cache — OpenInsider and FINRA data is intraday, not real-time
const ALT_CACHE = new Map()
const ALT_TTL_MS = 30 * 60_000
...
async function getAltDataSnippet(symbol) {
  ...
  const cached = ALT_CACHE.get(sym)
  ...
  ALT_CACHE.set(sym, { result, expiresAt: Date.now() + ALT_TTL_MS })
  return result
}

// ── Geopolitical Risk Scoring ─────────────────────────────────────────────────
// Scans recent market news headlines for geopolitical risk keywords and returns
// a scored snippet for AI Brain prompt injection.
// Source: Finnhub /news (keyless, general market news)

const GEO_PATTERNS = [
  { tier: 'SEVERE',   re: /war|armed conflict|invasion|airstr[ik]|missile strike|nuclear|coup|sanctions.*russia|sanctions.*china/i,       score: 90 },
  { tier: 'HIGH',     re: /sanctions|tariff hike|trade war|military tension|blockade|default|debt crisis|bank run/i,                       score: 70 },
  { tier: 'MODERATE', re: /tariff|election|protest|inflation surp|rate hike|earnings miss|supply chain|recession/i,                        score: 45 },
  { tier: 'LOW',      re: /merger|acquisition|ipo|earnings beat|dividend|buyback|partnership/i,                                            score: 15 },
]

const _geoCache = { ts: 0, snippet: null }
const GEO_TTL_MS = 60 * 60_000   // 1 hour

async function getGeopoliticalRiskSnippet(finnhubKey) {
  if (_geoCache.snippet && Date.now() - _geoCache.ts < GEO_TTL_MS) return _geoCache.snippet

  try {
    const url = `https://finnhub.io/api/v1/news?category=general${finnhubKey ? `&token=${finnhubKey}` : ''}`
    const r   = await safeFetch(url)
    if (!r?.ok) return null
    const articles = await r.json()
    if (!Array.isArray(articles)) return null

    const headlines = articles.slice(0, 30).map(a => a.headline || a.summary || '').filter(Boolean)

    let topTier = null, topScore = 0, topHeadline = ''
    for (const h of headlines) {
      for (const { tier, re, score } of GEO_PATTERNS) {
        if (re.test(h) && score > topScore) {
          topScore = score; topTier = tier; topHeadline = h
          break
        }
      }
    }
    ...
    const assets = topScore >= 70
      ? 'DXY↑ GOLD↑ OIL↑ Equities↓'
      : topScore >= 45
      ? 'Volatility↑ Defensives↑'
      : null

    const snippet = `GEOPOLITICAL RISK: ${topTier} (score ${topScore}/100) — "${topHeadline.slice(0, 80)}"${assets ? ` | Typical impact: ${assets}` : ''}`
    _geoCache.ts      = Date.now()
    _geoCache.snippet = snippet
    return snippet
  } catch {
    return null
  }
...
module.exports = { getInsiderActivity, getShortInterest, getAltDataSnippet, getGeopoliticalRiskSnippet }
