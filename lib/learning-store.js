...
function calibrationReport(decisions, { minSample = MIN_SAMPLE } = {}) {
  ...
  // Splits over surface-supplied meta. A persona or a model version is a
  // property of the DECIDER, not of the market, so a difference between two of
  // them is the one thing on this record that the system actually controls.
  //
  // Rows missing the field are excluded rather than pooled under 'unknown',
  // which would show up as a real persona with a real hit rate.
  const byMeta = (field) => {
    const out = {}
    for (const v of [...new Set(all.map(d => d?.meta?.[field]).filter(Boolean))]) {
      const c = calibrate(all.filter(d => d?.meta?.[field] === v), { minSample })
      if (c.sufficient) out[v] = c
    }
    return Object.keys(out).length ? out : null
  }
  const byPersona = byMeta('persona')
  const byModel   = byMeta('modelVersion')
  const byRegime  = byMeta('regime')
...
function buildCalibrationBlock(report) {
  ...
  const metaLine = (map, label, tail = '') => {
    if (!map) return
    const parts = Object.entries(map)
      .filter(([, c]) => c.sufficient)
      .map(([k, c]) => `${k} ${pct(c.alphaWinRate ?? c.winRate)} (n=${c.n})`)
    if (parts.length) lines.push(`${label}: ` + parts.join(' | ') + tail)
  }
  metaLine(report.byPersona, 'By investor persona',
    ' — measured, not stylistic. A persona below the overall rate is costing return, whatever it sounds like.')
  metaLine(report.byModel, 'By model version')
  metaLine(report.byRegime, 'By macro regime at decision time',
    ' — a rate that only holds in one regime is not an edge in the other.')
