...
async function revalidateLibrary({ headers = {}, limit = MAX_REVALIDATIONS } = {}) {
  ...
  for (const [symbol, group] of bySymbol) {
    ...
    for (const entry of group) {
      try {
        const { metrics } = simulate(timestamps, closes, entry.strategy, entry.params)
...
async function runEvolutionCycle({ headers = {}, date = new Date(), skipDiscovery = false } = {}) {
  ...
  const result = { startedAt, revalidation: null, discovery: null, outcomes: null, library: null, errors: [] }
  ...
  console.log(`[brain-evolution] cycle complete: revalidated=${result.revalidation?.revalidated ?? 0} retired=${result.revalidation?.retired ?? 0} discovered=${result.discovery?.stored ?? 0} outcomes=${result.outcomes?.resolved ?? 0} library=${result.library?.active ?? 0} active`)
  return result
}
...
module.exports = {
  ...
  // stages
  revalidateLibrary, discoverStrategies, resolveDecisionOutcomes, runEvolutionCycle,
}
