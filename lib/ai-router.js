...
class AIRouter {
  ...
  async call({ prompt, maxTokens = 4096, system = null, symbols = [] }) {
    // Claude paused (quota preservation): use Groq directly so the feature keeps
    // working; only error if Groq isn't configured either.
    if (claudePaused()) {
      if (process.env.GROQ_API_KEY) {
        console.warn(`[ai-router:${this.route}] Claude paused — routing to Groq`)
