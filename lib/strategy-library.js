...
function buildEntry({ symbol, strategy, params, name, rationale, marketFit, metrics, verdict, at = null }) {
...
function buildStrategyBlock(entries, { limit = 8 } = {}) {
  ...
  const lines = top.map(e => {
    ...
    return `  ${e.symbol}: ${e.strategy}(${paramStr}) — fitness ${e.fitness}, survived ${e.generation} backtest cycles, last alpha vs buy&hold ${alpha}, Sharpe ${last.sharpe ?? 'n/a'}`
  })
  return [
