...
const OVERRIDES_FILE = path.join(__dirname, '../data/brain-learnings-overrides.json')
...
function writeOverrides(patch = {}, { evidence = null, actor = 'human' } = {}) {
  ...
  ensureDataDir()
  fs.writeFileSync(OVERRIDES_FILE, JSON.stringify(next, null, 2))
...
function revertRefinement(seq, { reason = null } = {}) {
  ...
  try {
    ensureDataDir()
    fs.writeFileSync(LEARNINGS_FILE, JSON.stringify(snapshot, null, 2))
  } catch (e) {
...
async function resolveOutcomes() {
  ...
  const toResolve = records.filter(r => {
    ...
    return (age >= 7  * DAY && r.price7d  == null) ||
           ...
           (age >= 30 * DAY && r.barrier === undefined)
  })
  ...
  const updated = records.map(r => {
    ...
    if (copy.barrier === undefined && age >= 30 * DAY) {
      copy.barrier = tripleBarrier(bars, {
        from:   genMs,
        to:     genMs + 30 * DAY,
        target: r.targetZoneMid,
        stop:   r.stopLoss,
        long:   String(r.verdict || 'BUY').toUpperCase() !== 'SELL',
      })
    }
...
function computeStats(records) {
  ...
  // One calibration segment: filter, take the 30d return (7d when 30 is not
  // resolved yet) and its benchmark, and count. The longhand blocks below
  // predate this helper and are the same shape written out.
  ...
  // Asset-type calibration: crypto vs stock vs ETF (30d preferred).
  // Legacy records logged non-sector stocks as 'equity' — fold them into
  // 'stock' so one asset class never splits into two calibration segments.
  const assetTypeOf = r => (r.assetType === 'equity' ? 'stock' : r.assetType)
  const byAssetType = {}
  const assetTypes = [...new Set(resolved.map(assetTypeOf).filter(Boolean))]
  for (const at of assetTypes) {
    const rows = resolved
      .filter(r => assetTypeOf(r) === at && r.entered !== false)
      .map(r => ({
        ret:   returnsFor(r, r.price30d != null ? 'price30d' : 'price7d'),
        bench: r.price30d != null ? r.benchRet30d : r.benchRet7d,
      }))
  ...
  const byRegime = {}
  for (const rg of [...new Set(resolved.map(r => r.regimeAtScan).filter(Boolean))]) {
    const seg = segment(r => r.regimeAtScan === rg)
    if (seg) byRegime[rg] = seg
  }
  ...
  for (const g of [...new Set(resolved.map(genKey).filter(Boolean))]) {
    const seg = segment(r => genKey(r) === g)
    if (seg) byModelVersion[g] = seg
  }
  ...
  return {
    ...
    byModelVersion:      Object.keys(byModelVersion).length      ? byModelVersion      : null,
    barriers,
    autoTunedThreshold,
...
function getLearningsBlock() {
  try {
    ...
    const measuredLearnings = applyOverrides(data.keyLearnings, { ...overrides, pinned: [] })

    lines.push(
      '',
      'KEY LEARNINGS FROM PAST PREDICTIONS (derived from resolved outcomes):',
