...
/**
 * Date-aligned daily returns for two series of { date: 'YYYY-MM-DD', close }.
 * Returns [returnsA, returnsB] computed over common dates only, so mixed
 * calendars (crypto trades 7d/week, equities 5d/week) never shift against
 * each other — pairing returns by array position compares different days.
 */
function alignedReturnPair(a, b) {
  const bMap = new Map((b || []).map(p => [p.date, p.close]))
  const common = (a || [])
    .filter(p => p && p.date && p.close != null && bMap.has(p.date))
    .sort((x, y) => x.date.localeCompare(y.date))
  const ra = [], rb = []
  for (let i = 1; i < common.length; i++) {
    const pa = common[i - 1].close, ca = common[i].close
    const pb = bMap.get(common[i - 1].date), cb = bMap.get(common[i].date)
...
module.exports = {
  ...
  pearson, beta, alignedReturnPair,
