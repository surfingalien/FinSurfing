 ...
 *                     anchor's equity. PRIVATE anchors only: for a listed one
 *                     every index fund qualifies, which describes the fund
 *                     industry rather than an exposure map. For SpaceX or
 *                     OpenAI a listed fund's stake is the only equity exposure
 *                     that can be bought at all.
 *
...
/**
 * Forms that disclose what a registered fund owns.
 *
 * NPORT-P is the quarterly portfolio schedule (the primary document is XML);
 * N-CSR/N-CSRS are the annual/semi-annual shareholder reports, whose schedule
 * of investments is an HTML table and therefore the one that reads back as
 * text. Both are listed because EDGAR's form label for the N-PORT family has
 * appeared as both 'NPORT-P' and 'N-PORT', and a discovery path that hinges on
 * which spelling the index happens to use is a path that silently returns zero.
 */
const FUND_FORMS = ['NPORT-P', 'N-PORT', 'N-CSR', 'N-CSRS']
 ...
 *
 * Shared by both full-text discovery paths — they differ only in which forms
 * they search and what the resulting mention MEANS, never in how a hit becomes
 * a candidate.
 */
async function searchCandidates(anchorInfo, { forms, limitPerAlias = 40, fetchImpl = fetch, discovery = 'filing_search' } = {}) {
  ...
  for (const alias of anchorInfo.aliases) {
    ...
    for (const h of hits) {
      ...
      byCik.set(h.cik, {
        ...
        matchedAlias: alias, mentions: 1, discovery,
      })
    }
  }
  return [...byCik.values()]
...
/** Operating companies whose filings name the anchor: suppliers, customers, partners. */
function findMentions(anchorInfo, { forms = ['10-K', '20-F'], limitPerAlias = 40, fetchImpl = fetch } = {}) {
  return searchCandidates(anchorInfo, { forms, limitPerAlias, fetchImpl, discovery: 'filing_search' })
}

/**
 * Listed funds and BDCs whose portfolio disclosures name the anchor.
 *
 * This is the third discovery path, and the only one that reaches a PRIVATE
 * company. You cannot buy SpaceX; you can buy a listed closed-end fund or BDC
 * that holds SpaceX, and its NAV moves with the stake. That relationship is
 * written down quarterly in a schedule of investments, so it is discoverable on
 * exactly the same terms as a supply relationship in a 10-K.
 *
 * It refuses a LISTED anchor on purpose. Every index fund on the market holds
 * NVDA, so running this for a public anchor returns a ranked list of the fund
 * industry — technically correct, completely useless, and expensive. The caller
 * does not have to remember this; the guard lives here.
 */
async function fundHolders(anchorInfo, { forms = FUND_FORMS, limitPerAlias = 30, fetchImpl = fetch } = {}) {
  if (!anchorInfo || anchorInfo.listed) return []
  return searchCandidates(anchorInfo, { forms, limitPerAlias, fetchImpl, discovery: 'fund_holding' })
}
...
module.exports = {
  FTS_URL, FUND_FORMS, PRIVATE_ANCHORS, AMBIGUOUS_ALIASES,
  dedupeAliases, resolveAnchor,
  parseFtsHits, fullTextSearch, searchAvailable,
  searchCandidates, findMentions, fundHolders,
  findPeers,
}
