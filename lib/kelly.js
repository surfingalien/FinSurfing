 ...
 *   3. the overall resolved win rate (30d preferred, then 7d)
 *   4. a conservative fallback
 * Every level is gated on `minN` so a thin segment falls through to a broader
 * one rather than sizing off noise. Returns { p, source } to flag provenance.
 */
function winProbFromStats(stats, { confidence = null, assetType = null, fallback = 0.5, minN = 15 } = {}) {
  ...
  if (atSeg && typeof atSeg.winRate === 'number' && atSeg.n >= minN) {
    return { p: atSeg.winRate, source: `assetType:${atKey} (n=${atSeg.n})` }
  }
  ...
  // 30d horizon preferred, 7d when 30d hasn't accrued enough resolved picks.
  // A measured winRate of 0 is real evidence (fullKelly sizes p=0 to 0), never
  // a reason to substitute the more optimistic fallback.
  for (const [overall, label] of [[stats?.h30, '30d'], [stats?.h7, '7d']]) {
    if (overall && typeof overall.winRate === 'number' && overall.nTradeable >= minN) {
      return { p: overall.winRate, source: `overall ${label} win rate (n=${overall.nTradeable})` }
    }
  }
