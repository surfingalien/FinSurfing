...
function enrichPosition(pos, quote) {
  ...
  // Today's $ P&L. Count the day move for any FRESH quote (a stale last-known
  // fallback carries an old session's change, so it stays 0). Prefer prevClose,
  // then the provider's `change`, then back it out of changePct. We deliberately
  // do NOT require a same-day marketTime: most providers omit it, which zeroed
  // out almost every position and left Today's P/L far below the heatmap's day
  // moves (e.g. +$115 / "1↑ 0↓" instead of ~$2,390 across the whole book).
  const fresh = price !== null && !q.stale
  let todayGL = 0
  if (fresh) {
    if (prevClose != null)      todayGL = (price - prevClose) * pos.shares
