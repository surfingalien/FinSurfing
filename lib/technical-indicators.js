...
function detectPatterns(opens, highs, lows, closes, volumes) {
  ...
  // OBV divergence: accumulation (OBV up, price down) or distribution (OBV down, price up)
  if (volumes && n >= 22) {
    const obv = computeOBV(closes, volumes)
    if (obv) {
      const price20ago = closes[n - 21]
      const priceUp    = price > price20ago * 1.02
      const priceDown  = price < price20ago * 0.98
      if (priceDown && obv.trend === 'rising')  patterns.push('obv_bullish_divergence')
      if (priceUp   && obv.trend === 'falling') patterns.push('obv_bearish_divergence')
    }
  }

  // RSI divergence (last 30 bars, 14-period RSI)
  // Requires enough bars for a rolling RSI array
  if (n >= 44) {
    const lookback = Math.min(30, n - 14)
    const slice = closes.slice(n - lookback - 14)

    // Build rolling RSI for the lookback window
    const rsiArr = []
    for (let i = 14; i < slice.length; i++) {
      rsiArr.push(computeRSI(slice.slice(0, i + 1), 14))
    }

    const priceSlice = closes.slice(n - lookback)
    const halfLen = Math.floor(rsiArr.length / 2)

    if (halfLen >= 2) {
      const recentPrices = priceSlice.slice(halfLen)
      const recentRsi    = rsiArr.slice(halfLen)
      const prevPrices   = priceSlice.slice(0, halfLen)
      const prevRsi      = rsiArr.slice(0, halfLen)

      const recentHighIdx = recentPrices.reduce((mi, v, i) => v > recentPrices[mi] ? i : mi, 0)
      const prevHighIdx   = prevPrices.reduce((mi, v, i) => v > prevPrices[mi] ? i : mi, 0)
      const recentLowIdx  = recentPrices.reduce((mi, v, i) => v < recentPrices[mi] ? i : mi, 0)
      const prevLowIdx    = prevPrices.reduce((mi, v, i) => v < prevPrices[mi] ? i : mi, 0)

      // Bearish divergence: price higher high but RSI lower high
      if (
        recentPrices[recentHighIdx] > prevPrices[prevHighIdx] * 1.005 &&
        recentRsi[recentHighIdx] != null && prevRsi[prevHighIdx] != null &&
        recentRsi[recentHighIdx] < prevRsi[prevHighIdx] - 3
      ) patterns.push('bearish_rsi_divergence')

      // Bullish divergence: price lower low but RSI higher low
      if (
        recentPrices[recentLowIdx] < prevPrices[prevLowIdx] * 0.995 &&
        recentRsi[recentLowIdx] != null && prevRsi[prevLowIdx] != null &&
        recentRsi[recentLowIdx] > prevRsi[prevLowIdx] + 3
...
/**
 * Average Directional Index (ADX) — Wilder smoothing, 14-period default.
 * Returns 0-100 integer (>25 = trending, <20 = ranging/choppy) or null.
 */
function computeADX(highs, lows, closes, period = 14) {
  ...
  // Wilder initial sum then rolling: smooth[i] = smooth[i-1] - smooth[i-1]/period + arr[i]
...
// Decision-relevant pattern names — shared between compactTaLine (prompt injection)
// and the AI Brain prediction logger (pattern calibration via brain-learnings.js).
const KEY_PATTERNS = [
...
function compactTaLine(symbol, opens, highs, lows, closes, volumes) {
  ...
  const vwap   = volumes?.length ? computeVWAP(highs, lows, closes, volumes) : null
  const bb     = computeBB(closes)
  const obv    = volumes?.length ? computeOBV(closes, volumes) : null
  const patterns = detectPatterns(opens, highs, lows, closes, volumes)

  // 52-week (up to 252 bars) relative position: 0% = at 52w low, 100% = at 52w high
  const bars52  = Math.min(n, 252)
  const hi52    = Math.max(...highs.slice(n - bars52))
  const lo52    = Math.min(...lows.slice(n - bars52))
  const pos52w  = hi52 > lo52 ? Math.round((price - lo52) / (hi52 - lo52) * 100) : null
  ...
  const keyPatterns = patterns.filter(p => KEY_PATTERNS.includes(p))

  const parts = [
    rsi != null ? `RSI=${rsi}` : null,
    macd ? `MACD=${macd.trend}/${macd.histogramDir}` : null,
    trendBits.length ? `P${trendBits.join(',')}` : null,
    adx != null ? `ADX=${adx}${adx >= 25 ? '(trend)' : adx < 20 ? '(range)' : ''}` : null,
    vwap != null ? `VWAP%=${((price - vwap) / vwap * 100).toFixed(1)}` : null,
    bb != null ? `BB%B=${bb.pctB.toFixed(0)}${bb.squeeze ? '(squeeze)' : ''}` : null,
    pos52w != null ? `52w%=${pos52w}` : null,
    sr?.support    != null ? `S=${sr.support}`    : null,
    sr?.resistance != null ? `R=${sr.resistance}` : null,
    vol ? `Vol=${vol.ratio}x/${vol.trend}` : null,
    obv ? `OBV=${obv.trend}` : null,
...
// Compute intra-universe relative-strength percentile ranks from a Map of
// { symbol → 20-day return (%) }. Returns a new Map { symbol → 0-100 rank }
// where 100 = strongest performer in this scan universe, 0 = weakest.
// Returns empty Map for fewer than 2 symbols (rank is meaningless alone).
function computeRsRanks(ret20dMap) {
  if (!ret20dMap || ret20dMap.size < 2) return new Map()
  const sorted = [...ret20dMap.entries()].sort((a, b) => a[1] - b[1])
  const result = new Map()
  sorted.forEach(([sym], i) => {
    result.set(sym, Math.round(i / (sorted.length - 1) * 100))
  })
  return result
}
