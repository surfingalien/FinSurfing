...
- `recommendations.js` — AI Advisory; Claude primary + Groq fallback; accepts `persona` (see `lib/investor-personas.js`) + `includeMacro` + `includeFilings` (injects compact SEC filing narrative for focus stocks via `routes/filings.js`) body params; GET `/personas` returns persona list
...
- `filings.js` — `GET /api/filings/:symbol?form=10-K|10-Q|8-K`; SEC EDGAR filing-narrative reader (keyless) via `lib/filings.js` → AI router summary card (summary/keyChanges/riskFactors/managementTone/redFlags/analystTakeaway); 6h cache. Covers the 10-K/10-Q/8-K MD&A + Risk Factors narrative — distinct from `fundamentals.js` (numbers) and `earnings-call.js` (transcript). EDGAR needs a descriptive User-Agent (see `lib/filings.js:EDGAR_UA`)
...
## Extra AI providers (Z.ai / Qwen)
`lib/ai-router.js` `OPENAI_COMPAT` registry holds OpenAI-compatible providers — `groq` | `zai` (Z.ai/GLM, `https://api.z.ai/api/paas/v4`) | `qwen` (Alibaba DashScope intl, `https://dashscope-intl.aliyuncs.com/compatible-mode/v1`). Keys: `ZAI_API_KEY` `QWEN_API_KEY` (+ optional `ZAI_MODEL` `QWEN_MODEL` to override model names, which vendors rev often). `AI_FALLBACK_PROVIDER=groq|zai|qwen` (default `groq`) picks which one the router falls back to on Claude overload / pause. **`AI_PROVIDER_CHAIN`** sets an ordered failover chain for all model-driven research routes (`call()` walks it, auto-switching to the next provider on rate-limit/overload/5xx/circuit-open via `isSwitchable`; hard 4xx surface): e.g. `AI_PROVIDER_CHAIN=zai,qwen` runs research on GLM and fails over to Qwen, `zai,qwen,claude` adds Claude as last resort. Unset → default `claude,<AI_FALLBACK_PROVIDER>` (prior behaviour). Only configured providers are tried; Claude is dropped from the chain while `claudePaused()`. Base URLs are server-side only (SSRF-safe). Copilot (`routes/copilot.js` `PROVIDER_DEFAULTS` + `OPENAI_COMPAT_KEY`) exposes `zai`/`qwen` as selectable chat providers (`src/components/Copilot/FinSurfCopilot.jsx` `PROVIDERS`). NOTE: GLM/Qwen tool-calling + strict-JSON parity with Claude varies — test per feature.
