...
- Routes `routes/evolution.js` → `/api/evolution/status|strategies|paper|paper/trades`, admin-only `POST /run` and `/paper/reset`. Tests: `tests/strategy-library.test.js`, `tests/learning-store.test.js`, `tests/paper-broker.test.js`, `tests/brain-evolution.test.js`.
