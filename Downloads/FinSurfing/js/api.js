/* ═══════════════════════════════════════════════
   ...
   Sources: Yahoo Finance via CORS proxies
═══════════════════════════════════════════════ */
...
async function fetchSummary(symbol) {
  const modules = 'summaryDetail,financialData,defaultKeyStatistics,assetProfile,incomeStatementHistory';
...
async function fetchMultiQuote(symbols) {
  ...
  const url = `${YF}/v8/finance/quote?symbols=${encodeURIComponent(sym)}`;
  try {
    const data = await fetchWithProxy(url);
...
async function searchTicker(query) {
  const url = `${YF}/v1/finance/search?q=${encodeURIComponent(query)}&quotesCount=8&newsCount=0&enableFuzzyQuery=false&enableCb=false&enableNavLinks=false&enableEnhancedTrivialQuery=true`;
  try {
    const data = await fetchWithProxy(url);
...
// ─── Static screener dataset (S&P 500 leaders with approximate data) ───
